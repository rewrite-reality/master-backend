...
export class UsersService {
  ...
  private async clearUserCache(userId: string) {
    try {
      ...
      this.logger.warn(
        `Failed to clear cache for user ${userId}: ${(error as Error).message}`,
      );
    }
  }
    ...
    const finance = user.masterProfile
      ? (() => {
          ...
          return {
            ...
            debtLimit: limitDecimal.toNumber(),
            usagePercent,
  ...
  async createProfile(userId: string, dto: CreateProfileDto) {
    ...
    const uniqueDistrictIds = dto.districtIds
      ...
      : [];
    const patronymic =
      typeof dto.patronymic === 'string' ? dto.patronymic.trim() : '';
  ...
  async updateProfile(userId: string, dto: UpdateProfileDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!profile) {
        this.logger.warn(
          `Profile for user ${userId} not found in updateProfile`,
        );
      ...
      if (hasSpecialtyUpdate) {
        await tx.masterSpecialty.deleteMany({
  ...
  async getBalance(userId: string): Promise<BalanceResponseDto> {
    ...
    if (!master) {
      this.logger.warn(
        `Balance requested but master profile missing for user ${userId}`,
      );
      ...
      recentOrders: recentOrders.map((order) => ({
        ...
        district: order.district
          ? {
              ...
              city: order.district.city,
            }
          : null,
        specialty: order.specialty
          ? {
              id: order.specialty.id,
              name: order.specialty.name,
              code: order.specialty.code,
            }
          : null,
      })),
    };
  ...
  async updateMasterForAdmin(masterId: string, dto: AdminUpdateMasterDto) {
    if (
      dto.payoutPercent !== undefined &&
      (dto.payoutPercent < 1 || dto.payoutPercent > 100)
    ) {
