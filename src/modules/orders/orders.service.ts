...
export class OrdersService {
  ...
  private async getMasterProfile(
    userId: string,
  ): Promise<MasterProfileWithRelations> {
  ...
  private mapToDto(
    ...
    const isAssignedToCurrentMaster =
      !!currentMasterId &&
      !!order.masterId &&
      order.masterId === currentMasterId;
  ...
  async findAvailableOrders(
    ...
    const where: Prisma.OrderWhereInput = {
      ...
      districtId: query.districtId
        ? query.districtId
        : { in: masterDistrictIds },
      specialtyId: query.specialtyId
        ? query.specialtyId
        : { in: masterSpecialtyIds },
    };

    if (query.urgentOnly) {
      where.scheduledAt = {
        lte: new Date(Date.now() + 2 * 3600000),
        not: null,
      };
    }
    ...
    return orders.map((order) =>
      this.mapToDto(order as OrderWithRelations, master.id),
    );
  }
  ...
  async findOneById(
    ...
    const hasAccessByDistrict = master.districts.some(
      (d) => d.districtId === order.districtId,
    );
  ...
  async acceptOrder(
    ...
    if (cachedStatus && cachedStatus !== OrderStatus.PENDING) {
      ...
      throw new ConflictException(
        'Order was just accepted by someone else (Fast Check)',
      );
    }
    ...
    if (!hasAccessByDistrict || !hasAccessBySpecialty)
      throw new ForbiddenException(
        'Access denied: Outside district or specialty mismatch',
      );
    if (order.dispatchMode !== DispatchMode.RACE)
      throw new ForbiddenException(
        'This order is not available for Race acceptance',
      );
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (result.count === 0) {
        throw new ConflictException(
          'Order was just accepted by someone else or is no longer PENDING',
        );
      }
  ...
  async advanceOrderStatus(
    orderId: string,
    userId: string,
  ): Promise<{ id: string; status: OrderStatus; amoLeadId: string | null }> {
    const master = await this.getMasterProfile(userId);
    const order = await this.prisma.order.findUnique({
      where: { id: orderId },
      select: {
    ...
    if (order.masterId !== master.id)
      throw new ForbiddenException(
        'Only the assigned master can advance this order',
      );
  ...
  async submitForReview(
    ...
    if (order.masterId !== master.id) {
      throw new ForbiddenException(
        'Only the assigned master can submit review for this order',
      );
    }

    if (order.status !== OrderStatus.IN_PROGRESS) {
      throw new ConflictException(
        'Order must be in progress before submitting for review',
      );
    }
  ...
  async findAllForAdmin(query: AdminOrdersQueryDto) {
    ...
    return {
      items: items.map((order) =>
        this.mapAdminOrder(order as OrderWithRelations),
      ),
      total,
      limit: query.limit,
      offset: query.offset,
          ...
          meta: {
            ...
            changes: {
              ...
              price:
                dto.price ??
                (updated.price
                  ? new Prisma.Decimal(updated.price).toNumber()
                  : null),
