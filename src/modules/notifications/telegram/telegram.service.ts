...
export class TelegramService {
	...
	async sendOrderNotification(
		...
		try {
			const sentMessage = await this.bot.telegram.sendMessage(telegramId, message, {
				parse_mode: 'HTML',
				...Markup.inlineKeyboard([
					[
						// Кнопка 1: Моментальное действие (вызывает callback-хендлер в боте)
						Markup.button.callback(`⚡️ Принять за ${order.price}₽`, `accept_order:${order.id}`),
					],
					[
			...
			this.logger.log(`[TelegramService] Notification sent to ${telegramId} (msg: ${sentMessage.message_id})`);
	...
	async disableOrderButtons(telegramId: string | number, messageId: number) {
		try {
			await this.bot.telegram.editMessageReplyMarkup(telegramId, messageId, undefined, {
