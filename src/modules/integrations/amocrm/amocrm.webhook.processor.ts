...
export class AmoCrmWebhookProcessor extends WorkerHost {
  ...
  constructor(
    ...
    super();
    this.targetStatusId = Number(
      this.config.get<string>('AMOCRM_TARGET_STATUS_ID', '0'),
    );
  }
  ...
  private async handleStatusChange(lead: any) {
    try {
      ...
      this.logger.log(
        `Target status reached (${newStatusId}) for lead ${lead.id}, fetching details...`,
      );
  ...
  private async handleLeadAdd(lead: any) {
    try {
      ...
      this.logger.error(
        `Error processing lead add ${lead.id}: ${error.message}`,
        error.stack,
      );
      return { id: lead.id, status: 'error', error: error.message };
    }
  ...
  private async processLead(parsed: ParsedLeadData, originalId: string) {
    ...
    if (existingOrder) {
      this.logger.log(
        `Order already exists for Amo lead ${originalId}, skipping creation`,
      );
      return {
        id: originalId,
    ...
    if (!districtId) {
      ...
      if (anyDistrict) {
        ...
        this.logger.warn(
          `District '${address.district}' not found. Using fallback district '${anyDistrict.name}'.`,
        );
      } else {
        this.logger.error(
          `No districts found in DB. Cannot create order for lead ${originalId}.`,
        );
  ...
  private async enrichClientDetails(
    fullLead: any,
    ...
    if (!clientPhone && contactId) {
      ...
      if (contact) {
        ...
        this.logger.warn(
          `Could not resolve contact ${contactId} for lead ${parsed.amoId}`,
        );
      }
    }
