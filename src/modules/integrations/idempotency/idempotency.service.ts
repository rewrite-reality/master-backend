...
export class IdempotencyService {
	...
	async isDuplicate(leadId: string): Promise<boolean> {
		...
		try {
			const exists = await this.redis.set(key, '1', 'EX', this.TTL_SECONDS, 'NX');
			...
			this.logger.error(`Error checking idempotency for leadId ${leadId}: ${error.message}`, error.stack);
